...
export default function SetupPage() {
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      const res = await fetch("/api/setup/save-config", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sorobanRpcUrl: rpcUrl, databaseUrl: dbUrl, pollMs }),
      });
      const data = await res.json();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div>
        <h1 style={{ fontSize: 24, marginBottom: 6, display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          <div className="card" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <h3 style={{ fontSize: 16, borderBottom: "1px solid var(--border)", paddingBottom: 8 }}>
          ...
          <div className="card" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <p style={{ fontSize: 13, color: "var(--muted)" }}>
              Initialize database schema, apply table migrations, and populate mock transaction history (500+ events, 20 contracts).
            </p>
            ...
            <div>
              <button
                ...
                style={{ background: "var(--yellow)", color: "#0d1117", fontWeight: 700 }}
              >
          ...
          <div className="card" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <p style={{ fontSize: 12, color: "var(--muted)" }}>
              Open <a href="http://localhost:5173" target="_blank" rel="noreferrer">http://localhost:5173</a> to view the explorer interface.
            </p>
          </div>
        </div>
        ...
        <div className="card" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid var(--border)", paddingBottom: 8 }}>
            ...
            <button
              ...
              style={{ padding: "4px 10px", fontSize: 12, background: "var(--border)", color: "var(--text)" }}
            >
              Refresh
            </button>
          </div>
          ...
          {loadingDoctor ? (
            <p style={{ color: "var(--muted)", textAlign: "center", padding: 24 }}>Loading system diagnostic report…</p>
          ) : doctorError ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {/* Runtimes */}
              <div>
                <h4 style={{ fontSize: 13, color: "var(--muted)", marginBottom: 8, fontWeight: 600 }}>Runtimes</h4>
              ...
              <div style={{ borderTop: "1px solid var(--border)", paddingTop: 12 }}>
                <h4 style={{ fontSize: 13, color: "var(--muted)", marginBottom: 8, fontWeight: 600 }}>Database Connection</h4>
              ...
              {/* Ports */}
              <div style={{ borderTop: "1px solid var(--border)", paddingTop: 12 }}>
                <h4 style={{ fontSize: 13, color: "var(--muted)", marginBottom: 8, fontWeight: 600 }}>Ports Status</h4>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {Object.entries(report.ports).map(([port, check]) => (
                    <div key={port} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13 }}>
                      <span>Port {port}</span>
              ...
              {/* System Stats */}
              <div style={{ borderTop: "1px solid var(--border)", paddingTop: 12 }}>
                <h4 style={{ fontSize: 13, color: "var(--muted)", marginBottom: 8, fontWeight: 600 }}>System Resources</h4>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13 }}>
                  ...
                  <p style={{ fontSize: 11, color: "var(--muted)", marginTop: -4 }}>{report.system.disk.message}</p>
                  ...
                  <p style={{ fontSize: 11, color: "var(--muted)", marginTop: -4 }}>{report.system.memory.message}</p>
                </div>
              </div>

              {/* Git Hooks & Docker */}
              <div style={{ borderTop: "1px solid var(--border)", paddingTop: 12, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div>
                  <h4 style={{ fontSize: 13, color: "var(--muted)", marginBottom: 6, fontWeight: 600 }}>Git Hooks</h4>
                  <span className={getBadgeClass(report.gitHooks.status)}>
                    {report.gitHooks.status === "pass" ? "✓ Installed" : "⚠ Missing Hooks"}
                  </span>
                </div>
                <div>
                  <h4 style={{ fontSize: 13, color: "var(--muted)", marginBottom: 6, fontWeight: 600 }}>Docker Environment</h4>
